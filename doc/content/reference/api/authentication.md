...
{{< warning >}} When authorizing an API call, make sure to use the API key **value** and not the API key **ID**. Otherwise, you might face an error resembling `error:pkg/auth:token (invalid token)`. {{</ warning >}}
